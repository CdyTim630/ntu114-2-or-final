...
\begin{table}[htbp]
\centering\normalsize
\caption{Results for the 18 real-world instances. Gap is $(z_{\text{GRASP}}-z_{\text{MILP}})/|z_{\text{MILP}}|$. MILP returns global optima; GRASP-LS is 100\% hard-feasible with a 7.1\% average gap; Basic Greedy is 0\% feasible.}
\label{tab:real}
...
\begin{table}[htbp]
\centering\normalsize
\caption{Average performance across random-instance sizes, with five seeds per size. MILP objective is averaged over all five runs. Times are in seconds. GRASP-LS gap is the aligned average $(z_{\text{GRASP}}-z_{\text{MILP}})/|z_{\text{MILP}}|\times100\%$ over instances where both methods are feasible.}
\label{tab:rand}
\begin{tabular}{rrrrrrrr}
\toprule
$|I|$ & $|K|$ & MILP obj & MILP time & GRASP feas & GRASP gap & GRASP time & Greedy feas \\
\midrule
