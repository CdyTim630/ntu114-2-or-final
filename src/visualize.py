...
def fig_runtime_by_size():
    ...
    ax.set_xlabel("Number of items $|I|$")
...
def fig_beta_tradeoff():
    ...
    ax1.set_xlabel(r"Protein weight $\beta$")
    ax1.set_ylabel("Total cost (NT$)", color="#1f77b4")
    ax1.tick_params(axis="y", labelcolor="#1f77b4")
    ax2 = ax1.twinx()
    ax2.plot(betas, pros, "s--", color="#d62728", label="Total protein (g)")
    ax2.set_ylabel("Total protein (g)", color="#d62728")
    ax2.tick_params(axis="y", labelcolor="#d62728")
    ax1.set_title(r"Cost--protein trade-off vs. $\beta$")
    ax1.grid(alpha=0.3)
    plt.tight_layout()
...
def fig_gamma_diversity():
    ...
    ax1.set_xlabel(r"History-penalty weight $\gamma$")
    ax1.set_ylabel("Repeated recent items", color="#9467bd")
    ax1.set_ylim(0, 3.5)
    ax2 = ax1.twinx()
    ax2.plot(range(len(gammas)), costs, "o-", color="#ff7f0e", label="Total cost")
    ax2.set_ylabel("Total cost (NT$)", color="#ff7f0e")
    ax1.set_title(r"Effect of $\gamma$ on diversity")
    plt.tight_layout()
...
def fig_feasibility():
    ...
    ax.set_ylabel("Hard-feasible rate (%)", fontsize=12)
    ax.set_title("Feasibility on 18 real instances", fontsize=13, fontweight="bold")
    ax.set_ylim(0, 110)
...
def fig_price_of_diversity():
    ...
    ax1.set_xlabel("ε-best window (NT$)")
    ax1.set_ylabel("Distinct meals reachable", color="#1f77b4")
    ax1.tick_params(axis="y", labelcolor="#1f77b4")
    ax2 = ax1.twinx()
    ax2.plot(list(x), premium, "s--", color="#d62728", label="Avg. cost premium")
    ax2.set_ylabel("Avg. cost premium vs. optimum (NT$)", color="#d62728")
    ax2.tick_params(axis="y", labelcolor="#d62728")
    ax1.set_title("Price of diversity: variety vs. cost premium")
